...
class StockopnameController extends Controller
{
    ...
    public function actionDetail($id)
    {
        ...
        if(count($command) > 0){
            ...
            {
                $sqlCountStatus .= ", (SELECT COUNT(*) FROM stockopnamedetail WHERE stockopnamedetail.CurrentLocationID=locations.ID AND stockopnamedetail.StockOpnameID='2' AND stockopnamedetail.CurrentStatusID ='" .$item['ID'] . "') as 'JumlahStatus" . $item['Name']."'";
        ...
        if(Yii::$app->request->post('hasEditable'))
        {
            ...
            $keys = implode(array_keys(Yii::$app->request->post('Stockopnamedetail')[$index]));
            $new_val = array_shift(Yii::$app->request->post('Stockopnamedetail'))[$keys];
            // $post= [];
