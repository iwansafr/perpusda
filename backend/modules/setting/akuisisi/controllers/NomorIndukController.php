        ...
        if ($model->validate()) 
            {
                ...
                // echo '<pre>'; print_r($post['cbTemplate'][2]); die;
                if ($post['cbTemplate'][2] && $post['cbTemplate'][6] == 1) {
            ...
            return $this->redirect(['index']);
                ...
                if(strtolower($post['DynamicModel']['NomorInduk'])=='otomatis')
                {
                    foreach ($post['cbTemplate'] as $key => $dataFormat) {
                            ...
                            if($key==1&&  ($key == 0 || $key == 4 || $key == 8))
                            {
                                $post['cbTemplate'][$key] =  '{'.$post['cbTemplateInput'][$key].'}';
                            }
                            if($key==5&&  ($key == 0 || $key == 4 || $key == 8))
                            {
                    ...
                    foreach ($post['cbTemplate'] as $key => $dataFormat) {
                            ...
                            if($dataFormat==1&&  ($key == 0 || $key == 4 || $key == 8))
                            {
                                $post['cbTemplate'][$key] =  '{'.$post['cbTemplateInput'][$key].'}';
                            }
                            if($dataFormat==5&&  ($key == 0))
                            {
                                $post['cbTemplate'][$key] =  '^'.$post['cbTemplateInput']['51'].'^';
                            }
