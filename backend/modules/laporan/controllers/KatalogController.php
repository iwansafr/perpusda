...
public function actionRenderPdfKatalogPerkriteriaFrekuensi() 
    {
        ...
        // die;
            ...
            $sql = "SELECT 
                    ...
                    LEFT JOIN 
                    (SELECT 
                     locations.Name AS Kataloger,
                     ".$periode_location.",
                     collections.`Catalog_id` AS cata,
                     collections.`Location_id` AS loca
                     FROM collections 
                     INNER JOIN locations ON collections.Location_id=locations.ID
                     GROUP BY cata) test ON test.cata = catalogs.`ID`
                     LEFT JOIN locations ON test.loca=locations.ID
                     WHERE catalogs.CreateDate ";      
             $sql .= $sqlPeriode;
             $sql .= $andValue;
             if ($_POST['periode'] == "harian"){
                    $sql .= " GROUP BY DATE_FORMAT(catalogs.CreateDate,'%d-%m-%Y'),test.loca ORDER BY DATE_FORMAT(catalogs.CreateDate,'%Y-%m-%d') DESC";
                } elseif ($_POST['periode'] == "bulanan") {
                    $sql .= " GROUP BY MONTH(catalogs.CreateDate),test.loca ORDER BY DATE_FORMAT(catalogs.CreateDate,'%Y-%m-%d') DESC";
                } else {
                    $sql .= " GROUP BY YEAR(catalogs.CreateDate),test.loca ORDER BY DATE_FORMAT(catalogs.CreateDate,'%Y-%m-%d') DESC";
                }   
            }
            ...
            if (implode($_POST['kriterias']) == 'bahan_pustaka') {
             ...
             if (implode($_POST['kriterias']) == 'subjek' || isset($_POST['no_klas'])) {
                ...
                if ($_POST['periode'] == "harian"){ 
                      ...
                      $sql .= "GROUP BY kelas, DATE_FORMAT(cat.CreateDate,'%M-%Y') ";
                } else{
                       $sql .= "GROUP BY kelas, DATE_FORMAT(cat.CreateDate,'%Y') ";
                }
            }
